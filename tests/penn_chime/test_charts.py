...
def test_admits_chart(admits_floor):
    chart = build_admits_chart(alt=alt, admits_floor=admits_floor)
...
def test_build_descriptions(admits_floor, param):
    chart = build_admits_chart(alt=alt, admits_floor=admits_floor)
...
def test_no_asterisk(admits_floor, param):
    param.n_days = 600
...
def test_census(census_floor, param):
    chart = build_census_chart(alt=alt, census_floor=census_floor)
    description = build_descriptions(chart=chart, labels=param.labels)
...
def test_census_chart(census_floor):
    chart = build_census_chart(alt=alt, census_floor=census_floor)
    assert isinstance(chart, (alt.Chart, alt.LayerChart))
