...
admits_chart = build_admits_chart(alt=alt, admits_floor=m.admits_floor, max_y_axis=p.max_y_axis)
st.altair_chart(admits_chart, use_container_width=True)
st.markdown(build_descriptions(chart=admits_chart, labels=p.labels, prefix="", suffix=" Admissions"))
display_download_link(
    st,
...
census_chart = build_census_chart(alt=alt, census_floor=m.census_floor, max_y_axis=p.max_y_axis)
st.altair_chart(census_chart, use_container_width=True)
st.markdown(build_descriptions(chart=census_chart, labels=p.labels, prefix="", suffix=" Census"))
display_download_link(
    st,
...
sim_sir_chart = build_sim_sir_chart(alt=alt, sim_sir_floor=m.sim_sir_floor)
st.altair_chart(sim_sir_chart, use_container_width=True)
display_download_link(
    st,
    filename=f"{p.current_date}_sim_sir.csv",
    df=m.sim_sir,
)
